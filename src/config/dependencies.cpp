...
//    std::cerr << "Processing '" << c << "'";
    ...
    if (c == "none")      continue;
      ...
      if (!neg)  splitKeys[s].insert(p);
        ...
        if (prefixes.empty())
          for (const std::string &p_: projects) prefixes.insert(p_);
        prefixes.erase(p);
      }
    } else
...
bool Dependencies::compareStates (const ConfigIterator &previous,
  ...
  for (const auto &key: keys) {
    ...
    lhsStr = toString(lhsIT->second);
    rhsStr = toString(rhsIT->second);
